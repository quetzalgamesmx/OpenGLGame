...
void OShaderProgram::attach(const wchar_t* shaderFilePath, const OShaderType& type)
{
	...
	if (logLenght > 0) {
		vector<char> errorMessage(logLenght + 1);
		glGetShaderInfoLog(shaderId,logLenght,NULL,&errorMessage[0]);
		OGL3D_WARNING("OShaderProgram | "<<shaderFilePath<< " compiled with errors"<<endl<<&errorMessage[0]);
		return;
	}


	glAttachShader(m_programId,shaderId);
	m_attachShaders[(ui32)type] = shaderId;

	OGL3D_INFO("OShaderProgram | " << shaderFilePath << " compiled succesfully" << endl);
}
